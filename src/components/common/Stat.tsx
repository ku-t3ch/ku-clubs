...
const Stat: NextPage<Props> = ({ likes, views, location, size, clubData, clicked, canlike = true }) => {
    ...
    const onClickLike = () => {
        ...
        if (!clickedIn && clubData?.id) {
            likeApi.mutate(
                ...
                {
                    onSuccess: () => {
                        toast.success("Liked!", { id: keyLoading });
            ...
            likeApi.mutate(
                ...
                {
                    onSuccess: () => {
                        toast.success("Unliked!", { id: keyLoading });
                        apiGetLikeAmount.refetch();
                        setClicked(false);
                    },
                }
            ...
            {canClickLike ? (
                <>
                    {clickedIn ? (
                        ...
                        <div
                            className={clsx("flex items-center", "cursor-pointer", canlike ? "hover:text-blue-400" : "")}
                            onClick={onClickLike}
                        >
                            <Icon icon="material-symbols:thumb-up-outline" className={clsx(sizeIcon)} />
                            <div className={clsx(font, "ml-1")}>{apiGetLikeAmount.data || likes} likes</div>
